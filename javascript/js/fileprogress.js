...
function FileProgress(file, targetid)
{
    ...
    if (!this.fileProgressElement)
    {
        ...
        this.filenameA.innerHTML = "删除";
        this.filenameA.href = "javascript:void(0)";
        this.filenameA.style.display = "none";
        //this.filenameA.style.margin = "0 10px 0 5px";
        this.fileProgressElement.appendChild(this.filenameA);
        ...
        this.filenameSpan = document.createElement("a");
        this.filenameSpan.innerHTML = file.name;
        this.fileProgressElement.appendChild(this.filenameSpan);

        //delete href
        this.hrefSpan = document.createElement("span");
        this.hrefSpan.style.cursor = "pointer";
        this.hrefSpan.style.marginLeft = "20px;"
...
{
    this.stateButton = this.fileProgressElement.childNodes[5];
    this.fileSpan = this.fileProgressElement.childNodes[3];//存放名称
    this.stateDiv = this.fileProgressElement.childNodes[1];//进度条
    //this.
    this.stateBar = this.stateDiv.childNodes[0];//百分比
    this.percentSpan = this.fileProgressElement.childNodes[2];//百分比
    this.hrefSpan = this.fileProgressElement.childNodes[4];
    this.hrefControl = this.hrefSpan.childNodes[0];

    this.filenameA = this.fileProgressElement.childNodes[0];
...
percent:     已上传百分比
*/
FileProgress.prototype.setProgress = function(percent,file)
{
    this.stateBar.style.width = percent + "%";
    this.percentSpan.innerHTML = percent + "%&nbsp;" + SWFUpload.speed.formatBytePS(file.averageSpeed);
    ...
    if (percent == 100)
    {
        ...
        this.hrefControl.style.display = "none";
